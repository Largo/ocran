...
# it still meets the expectations of Ocran which only needs the appended "signature" to match
# the size and location as specificed in the header.
#
# The purpose of this class is to create a "signed" executable so that we can test Ocran's
# ability to build executables that are resilient to authenticode code signing, i.e that
# Ocran executables still work after they've been digitally signed.
#
...
class FakeCodeSigner
  ...
  def sign
    if pe_wrapper.security_entry_valid?
      raise "Binary already signed with valid signature, nothing to do!"
    end

    # Reset any invalid security entry data
    if pe_wrapper.security_size != 0
      pe_wrapper.security_size = 0
      pe_wrapper.security_address = 0
    end
